...
class BkmExpressPaymentService {
    constructor(keyPath, wsdl) {
        ...
        this.request = ExpressPaymentRequest;
        this.response = ExpressPaymentResponse;
        this.expressPaymentResultCodes = ExpressPaymentResultCodes;
    }

    expressPayment(request, callback) {
        const wsdlOptions = {
            ignoredNamespaces: {
        ...
        soap.createClient(this.wsdl, wsdlOptions, function (err, client) {
            client.initializePayment(
                { 'tns:initializePaymentWSRequest': request },
                callback,
                { strictSSL: false }
            );
        });
